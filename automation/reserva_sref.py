...
def preencher_sala(navegador, wait, valor_sala):
    ...
    campo = wait.until(
        EC.presence_of_element_located((By.ID, "sala"))
    )
    ...
    if not existe:
        ...
        raise ValueError(
            ...
            f"para o tipo selecionado. Opções: {opcoes}"
        )
    ...
    if campo.get_attribute("value") != valor_sala:
        raise RuntimeError(
            f"Sala não foi preenchida corretamente: {valor_sala}"
        )
...
def marcar_dias(navegador, wait, dias):
    ...
    for dia in dias:
        ...
        if not checkbox.is_selected():
            raise RuntimeError(
                f"Não foi possível selecionar {dia}"
            )


def preencher_reserva(navegador, wait, reserva):

    # O Selenium usa SOMENTE os campos normalizados.
    #
    # espaco_original NÃO é utilizado aqui.
    #
    # O funcionário pode alterar a Sala SREF
    # independentemente do espaço original.

    tipo_sala = reserva.get(
        "tipo_sala",
        ""
    ).strip()
    ...
    if not tipo_sala or not sala:
        raise ValueError(
            "tipo_sala e sala são obrigatórios."
        )
    ...
    # Aguarda as opções de sala carregarem
    wait.until(
    ...
    # RESERVADO PARA
    reservado = (
        reserva.get("reservado_para")
        or reserva.get("solicitante")
        or ""
    )
    ...
    campo.clear()
    campo.send_keys(reservado)

    # DESCRIÇÃO
    descricao = reserva.get(
        "descricao",
        ""
    )
